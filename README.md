...
- **GitHub Copilot login** (official partnership, Jan 2026): Copilot
  ...
  (open, Sep 10 2026) — legacy _per-request_ Copilot plans can drain an entire
...
- **Billing-dispute caveat:** issue
  ...
  reports the Console's _global_ monthly usage hitting 100% while the sum of
...
- **Abuse monitoring / client requirements:** send coding-agent-style traffic,
  ...
  **GitHub Copilot Chat** under _Known problematic clients_ (session header missing;
...
> the v2 beta) described a distinct `opencode2` binary that _could_ run
...
5. When ready, convert config to the native v2 shape (optional, can be done
   ...
   _"Migrate my OpenCode configuration, including file-based definitions, from the
...
- **Match the agent to the scenario, not just the price.** §5 now ships six
  ...
  - `vision` (`deepseek-v4-flash-vision-exp`) — the one model Go documents
    ...
    ordinary text turns. It's the safe default, not necessarily the _only_
...
**Which model for search-heavy work?** The search _backend_ is model-independent;
...
- **ACP for everything else.** `opencode acp` speaks Agent Client Protocol over
  ...
  _not_ join the shared background service), and can serve multiple sessions.
...
- **Inline completions are not an OpenCode feature** (it's an agent, not a
  ...
  1. **No abuse-monitoring identification.** §2 says Go expects coding-agent
     ...
     _verified_ list either (Hermes, Claude Code, Codex, ZCode, Pi, jcode, Kilo
  ...
  In short: it works as a way to spend the _same_ Go dollars from a second client,
...
OpenCode has no claude.ai-style artifact pane, but it can _produce real document
files_ the same way Claude's web UI does under the hood — by running code — and v2's
...
3. Consume the results anywhere: open them with normal apps, commit them, or export
   ...
   session-sharing link yet (§1), so share the _artifacts_, not the session.
...
- **Go model lineup and pricing** — fetched <https://opencode.ai/docs/go/> directly
  ...
  in "current list of models," usage-limits, estimated-requests, _or_ Privacy) —
...
- **⚠ Version discrepancy — worth running `opencode --version` yourself.** The
  ...
  **Independently reconfirmed 2026-09-23:** a direct re-fetch of
  ...
  That same intro page, however, _does_ list a full platform matrix straight
  ...
  this guide's fetch found, that's an inconsistency _between two of Anomaly's
  own pages_, not just a stale draft in this guide. Practical takeaway: for any
...
from both the "current list of models" bullet _and_ the privacy table — this
...
re-checking the _live_ pages rather than trusting any single snapshot — this
one included.
